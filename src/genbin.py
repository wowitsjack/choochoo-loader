...
This script writes out the complete codebase for the ChooChoo project.
It targets .NET Framework 4.8 (for Wine Mono), uses a dark theme with all text in white
(using the built‑in "Tahoma" font), enumerates processes in a Wine‑friendly way, and
integrates a fully overhauled in‑app file browser and TV Mode. In TV mode the UI is forced
fullscreen and shows a header (with a slim Launch button), a centered menu with improved spacing/padding,
and a status panel at the bottom displaying current configuration and console output.
All functions available in desktop mode (such as picking files, launching processes, configuring DLL injections,
managing profiles, etc.) are re‑implemented in a TV‑friendly interface that works all in one window.

All preloader/launcher functionality has been removed so that the application is a standalone window.
The MainForm overrides activation to call ReleaseCapture and clear any cursor clipping.
We also load our own custom cursor (from "cursor.cur" if available) and install global low‑level keyboard and mouse hooks
(via GlobalHook.cs and GlobalMouseHook.cs) so that our application always shows its own cursor and captures input
even when an exclusive fullscreen game is active.

The process spawner has been reworked with a retry loop for robustness.
The application auto‑saves and restores the last used paths from "profiles\\last.ini" on launch.
In TV mode, pressing "Launch Application" immediately starts the launch/inject process while keeping TV mode active.
When launching the Trainer, if no new process is observed within 3 seconds, the trainer exe
will be launched via a quiet command‑line call.

Additionally, a set of six radio buttons has been added under the Launch button in desktop mode to choose
different trainer launch methods (using techniques such as P/Invoke CreateProcess, CMD, CreateThread Injection,
Remote Thread Injection, Shell Execute, and Raw Process.Start). DLL injection has been improved so that if a valid DLL is specified,
its details (including bitness) are printed and injection (via LoadLibraryW) is performed immediately if the “Inject”
checkbox is ticked.
